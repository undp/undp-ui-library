...
function MessageAvatar({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex w-fit min-w-8 shrink-0 items-center justify-center self-end overflow-hidden rounded-full bg-surface group-has-data-[slot=message-footer]/message:-translate-y-8',
        className,
      )}
      {...props}
    />
...
function MessageHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex min-w-0 max-w-full items-center px-3 font-medium text-tertiary text-xs group-has-data-[variant=ghost]/message:px-0',
        className,
      )}
      {...props}
    />
